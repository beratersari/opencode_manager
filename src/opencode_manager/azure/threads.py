"""Map findings onto Azure PR threads. Same ExistingThread type as GitLab."""
...
from opencode_manager.review.threads import ExistingThread, is_creasy_finding_body
from opencode_manager.workspace.diffmap import DiffMap
...
def parse_azure_thread(raw: dict[str, Any]) -> Optional[ExistingThread]:
    ...
    if not is_creasy_finding_body(body):
        return None
    ...
    return ExistingThread(
        ...
        side=side,
        resolved=resolved,
        last_body=_last_creasy_body(comments),
        root_comment_id=_as_line(first.get("id")),
    )


def _last_creasy_body(comments: list) -> str:
    last = ""
